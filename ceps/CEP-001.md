...
Raised in [#147](https://github.com/BrightbeamAI/chap/pull/147) and recorded
here as a contributed argument rather than an editorial position. The open
question above stays open; merging this discussion does not decide it.
...
approved. The Coordinator refuses mid-review substitution (see "The state
machine"), so the artefact cannot change underneath an open review; the gap is
that a relying party cannot establish from the signed decision alone what was
approved, and must trust the Coordinator's chain for it. That is the same shape
as the advertised-versus-enforced gap the profile-semantics work addresses: the
signature looks stronger than it is. Requiring the digest under
`require_signatures` makes the signature bind the thing decided.
