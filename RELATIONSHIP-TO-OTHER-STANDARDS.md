# HAP's Relationship to Other Standards
...
HAP concept to its nearest existing standard and states whether HAP
reuses, profiles, or diverges from it — and why.

If you're reviewing HAP's design, **read this document first**.
...
| HAP concept                  | Existing standard                                | Relationship |
|------------------------------|--------------------------------------------------|--------------|
...
| URIs                         | **RFC 3986**                                     | Plain URIs; HAP defines the scheme grammar. |
...
The only things HAP introduces that don't exist elsewhere are
...
HAP envelopes are valid [JSON-RPC 2.0](https://www.jsonrpc.org/specification)
messages. Specifically:

| HAP field        | JSON-RPC equivalent             |
|------------------|---------------------------------|
| `id`             | `id`                            |
| `method`         | `method`                        |
| `params`         | `params`                        |
...
HAP adds a small fixed set of fields (`hap`, `workspace`, `from`, `to`,
...
A HAP envelope from any conformant implementation passes a generic
JSON-RPC 2.0 validator. The extra HAP-specific fields are present
...
When messages must be hashed or signed, HAP canonicalises them with
...
4. SCITT also uses it, so HAP's `audit-scitt` profile composes naturally.

HAP does not define its own canonicalisation rules.
...
The HAP innovation isn't the diff itself — it's the **rationale +
...
HAP has no native identity layer. It defines two profiles, each of
...
binds the HAP signing key. Step-up authentication uses standard
...
SVIDs. The HAP signing key is bound to the SPIFFE ID via the mesh's
...
HAP's audit defers to a mature, IETF-tracked standard rather than
...
HAP's `audit-scitt` profile says: **the workspace's evidence chain
is a SCITT transparency log.** Specifically:

- Each HAP message becomes a SCITT signed statement.
...
- Auditors verify receipts against the transparency service's signed
  log root; HAP doesn't define a parallel verification path.
...
HAP doesn't re-implement either MCP or A2A. Composition is by
**citation**, not encapsulation:

- An MCP tool call called by an agent during HAP work becomes a
  `citation` of kind `mcp_tool_invocation` inside the agent's HAP
  artefact, with `input_hash` and `output_hash` providing the
  hash boundary. See [`integrations/HAP-with-MCP.md`](./integrations/HAP-with-MCP.md).
- An A2A peer is represented inside a HAP workspace by a
  `service:bridge…` Participant. Cross-org work goes via that
  Participant; the A2A traffic does not cross the HAP wire. See
  [`integrations/HAP-with-A2A.md`](./integrations/HAP-with-A2A.md).

The MCP and A2A protocols evolve on their own timelines; HAP's
...
"my org's HAP workspace can subscribe to your org's HAP workspace's
events"), the planned `federation-activitypub` profile (v0.2 draft)
maps:

| HAP concept       | ActivityPub concept |
|-------------------|---------------------|
| Workspace         | Actor                |
| Participant       | Actor                |
...
events to HAP `participant.join` and `participant.leave`.
...
HAP is transport-agnostic. The recommended bindings are:
...
HAP fields) is identical across them. Existing standards-based
...
HAP-specific transport plumbing.

---

## 11. URIs

HAP Participant URIs use [RFC 3986](https://datatracker.ietf.org/doc/html/rfc3986)
generic syntax. The HAP-specific schemes (`human:`, `agent:`,
`service:`, `group:`, `workspace:`) are registered under a single
parent scheme `hap:` for IANA registration purposes; see
...
When an implementation attests to conformance with a HAP level
...
with subject `hap-implementation:<name>:<version>` and predicate
`hap.dev/conformance/v1`. This lets standard supply-chain tooling
discover and verify HAP conformance claims.

---

## 14. What HAP introduces that doesn't exist elsewhere

Stripped of the reused standards, HAP introduces:
...
These are HAP's actual contribution to the protocol ecosystem.
...
| Layer            | HAP's contribution                            | Source of standards |
|------------------|-----------------------------------------------|---------------------|
...
| Methods          | **The 7 Core + 20+ profile methods**          | HAP itself |
| Override shape   | **The override-with-rationale primitive**     | HAP itself |

HAP is, deliberately, a thin layer of well-chosen verbs on top of
a deep stack of existing standards. If you find yourself
reinventing one of the rows above in your HAP implementation,
