...
def test_wrap_mcp_records_fulfils_when_given() -> None:
    coord = _make_coord()
    res = wrap_mcp_tool_call(
        coord, "wsp_wrap",
    ...
    assert task.output["fulfils"] == "art_approved_decision"


def test_wrap_mcp_omits_fulfils_by_default() -> None:
    coord = _make_coord()
    res = wrap_mcp_tool_call(
        coord, "wsp_wrap",
        caller="agent:bot", tool="some.tool",
    ...
    task = coord.get_workspace("wsp_wrap").tasks[res["task_id"]]
    assert "fulfils" not in task.output


def test_wrap_a2a_records_fulfils_when_given() -> None:
    coord = _make_coord()
